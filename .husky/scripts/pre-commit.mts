...
const updateBlog = async (blog: BlogUpdate[]) => {
  try {
    ...
    if (resp.status !== 200) {
      throw await resp.json() as string
    } else {
...
const updateMeta = async (meta: MetaUpdate[]) => {
  try {
...
const files = (
  await $`git diff --cached --name-only --diff-filter=ACM`
).stdout.split('\n').filter(Boolean)
